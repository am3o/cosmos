...
# docker buildx build --tag cosmos --file .pi/Dockerfile --platform linux/arm64 --cache-from cosmos:cache --cache-to cosmos:cache --load --progress tty .

# save the image to result-rootfs.tar
docker save --output ./.tmp/result-rootfs.tar cosmos
...
sudo bash -c "echo cosmos > ./.tmp/result-rootfs/etc/hostname"
...
# Make the image file
fallocate -l 4G "images/cosmos.img"

# loop-mount the image file so it becomes a disk
sudo losetup --find --show images/cosmos.img
