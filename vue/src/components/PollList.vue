...
export default {
  ...
  computed: {
    polls() {
      return this.$store.state.cosmos.data["cosmos/poll"] || [];
    },
    votes() {
      return this.$store.state.cosmos.data["cosmos/vote"] || [];
    }
  },
  methods: {
    results(id) {
    ...
    async submit(pollID, value) {
      ...
      await this.$store.dispatch("cosmos/entitySubmit", { ...type, module:"cosmos", body });
      await this.$store.dispatch("cosmos/entityFetch", {...type, module: "cosmos"});
    }
  }
};
